...
##Work Log:
  - 2025/11/20, Work commenced.
    Cloned the repository for this task and created it on my GitHub.
  - 2025/11/22~23
     Downloaded Git from [https://git-scm.com/install/windows] and connected GitHub to VSCode. However, file synchronization, pull, and push did not function. Finally resolved by entering the following text in the terminal:
    ```text
     git status
    ...
    ```
  - 2025/11/24
     Attempted to execute the created hash, but the code—which should have been updated on GitHub—remained unchanged in the       terminal, rendering the code inoperable.
  - 2025/11/29~30
    ```text
      C:\Users\11401_CS203A\Assignment\AssignmentIV>git remote -v origin  https://github.com/yfhuang/11401_CS203A.git(fetch) origin  https://github.com/yfhuang/11401_CS203A.git (push)
    ```
     The cause was identified and fixed (changed from yfhuang to 2Ryo1). After that, the code was re-fetched and became executable.
...
## Results
- The collision rate shall be calculated using the following formula as a reference.
- Collision rate = (number of keys that share an index with at least one othe) / (total number of keys)
...
| 37             | 10%             | Distributed over a wide area, the collision has been  resolved             |
